...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        plot_name='relu forward',
        args={'num_rows': 32},
        ylabel='milliseconds',
        x_log=True
    )
)
def bench_relu_forward(num_rows, num_cols, provider):
    x = torch.randn(num_rows, num_cols, device='cuda')

    if provider == 'torch':
        return triton.testing.do_bench(lambda: torch.nn.functional.relu(x))
    else:
        return triton.testing.do_bench(lambda: trident.function.relu(x))
