...
@util.report("silu forward", ["x_size"], [128 * i for i in range(1, 21)], {"y_size": 16})
def bench_silu_forward(y_size, x_size, dtype, backend):
...
@util.report("silu backward", ["x_size"], [128 * i for i in range(1, 21)], {"y_size": 16})
def bench_silu_backward(y_size, x_size, dtype, backend):
...
def run_benchmark(mode, show_plots, dtype):
    if mode == "forward":
        bench_silu_forward.run(print_data=True, show_plots=show_plots, dtype=dtype)
    else:
        bench_silu_backward.run(print_data=True, show_plots=show_plots, dtype=dtype)
