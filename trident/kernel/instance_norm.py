...
class InstanceNorm:
    ...
    def backward(
        ...
        inp = triton.language.load(inp_ptr + blk, msk, 0.0)

        if running_mean_ptr:
            mean = triton.language.load(running_mean_ptr + ch)
        else:
            mean = triton.language.sum(inp, 0) / vec_sz

        if running_var_ptr:
            var = triton.language.load(running_var_ptr + ch)
        else:
            var = language.var(msk, inp, vec_sz, mean, corr=0)

        std = language.std(var, eps)
        grad_out = triton.language.load(grad_out_ptr + blk, msk, 0)
        wgt = triton.language.load(wgt_ptr + pid) if wgt_ptr else 1
        ...
        grad_xc = grad_xn / std + (2.0 / vec_sz) * xc * grad_var
        grad_mu = triton.language.sum(triton.language.where(msk, grad_xc, 0.0))
        grad_inp = grad_xc - grad_mu / vec_sz
        ...
        #     a = grad_out * wgt
        #     d = triton.language.sum(grad_out * wgt, 0) / vec_sz
        #     d = language.gemv(grad_out, wgt) / vec_sz
        # else:
        #     a = grad_out
        #     d = language.sum(grad_out) / vec_sz
        #
        # b = triton.language.where(msk, (inp - mean) / std, 0)
