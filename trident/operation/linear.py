...
class Linear(torch.autograd.Function):
    ...
    def __forward(inp, wgt, bis, act):
        ...
        if bis is None:
            ...
            kernel.linear[grid](
                ...
                wgt.stride(1),
                bis,
                bis.stride(0) if bis.dim() == 1 else bis.stride(1),
                m,
                k,
                n,
                act=act,
    ...
    def __backward(grad_out, inp, wgt, out, act):
        grad_act = grad_out * Linear.__activation_grad(act, out)
    ...
    @staticmethod
    def __activation_grad(act, out):
        if act is None:
            return 1
        elif act == "relu":
            return torch.where(out > 0, 1, 0)
        elif act == "leaky_relu":
            return torch.where(out > 0, 1, 1e-2)
